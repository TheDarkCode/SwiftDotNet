...
namespace SwiftDotNet.Crawlers
{
    public static class ViewDNS
    {
        public static async Task<List<string>> CrawlReverseDomainsforNameServer(string nameserver)
        {
            ...
            var resultString = await _webClient.GetStringAsync(nameserverUri);
        ...
        public static async Task<bool> CrawlIsSiteDown(string domain)
        {
            ...
            var resultString = await _webClient.GetStringAsync(domainUri);
